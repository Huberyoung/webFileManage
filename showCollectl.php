...
//获得该用户的所有收藏
if($type==1){
    $sql='SELECT c.`id`,users.name AS usersName,c.`user_id`,upload.name AS uploadName, c.`upload_id`, c.`create_time` FROM `collect` AS c,upload,users WHERE users.id=c.user_id AND upload.id=c.upload_id AND  c.user_id='.$user['id'].' ORDER BY create_time DESC LIMIT 0,10';
}else{
    $sql='SELECT c.`id`,users.name AS usersName,c.`user_id`,upload.name AS uploadName, c.`upload_id`, c.`create_time` FROM `collect` AS c,upload,users WHERE users.id=c.user_id AND upload.id=c.upload_id AND  c.user_id='.$user['id'].'  LIMIT 0,10';
}
$t=$s->select($sql);
$collects=$t['result'];
?>
            ...
            <div class="col-md-12 column">
                <div class="tabbable" id="tabs-760076">
                    <ul class="nav nav-tabs">
                        <li>
                            <a rel="nofollow" href="showIndividual.php ? id=<?php echo $id;?>" >资源列表</a>
                        </li>
                        <li>
                            <a rel="nofollow" href="showIndividual.php ? id=<?php echo $id;?>">分享列表</a>
                        </li>
                        <li class="active">
                            <a rel="nofollow" href="#panel-243720" data-toggle="tab">收藏列表</a>
                        </li>
                        <li class="dropdown pull-right">
                            <a rel="nofollow" href="#" data-toggle="dropdown" class="dropdown-toggle">排序<strong class="caret"></strong></a>
                            <ul class="dropdown-menu">
                                <li>
                                    <a rel="nofollow" href="./showCollectl.php ? id=<?php echo $id;?>">时间升序</a>
                                </li>
                                <li>
                                    <a rel="nofollow" href="./showCollectl.php ?id=<?php echo $id;?> && type=1">时间降序</a>
                                </li>
                            </ul>
                        </li>
                    </ul>
                                    ...
                                    <div class="col-md-12 column">
                                        <table class="table table1">
                                            ...
                                            <?php foreach($collects as $v):?>
                                                ...
                                                <?php endif;?>
                                                <td>
                                                    <a href="fileDetail.php?uploadId=<?php echo $v['upload_id'];?>"> <?php echo $v['uploadName'];?></a>
                                                </td>
                                                <td>
                                                    <a href="showIndividual.php?id=<?php echo $v['user_id'];?>"><?php echo $v['usersName'];?></a>
                                                </td>
                                                <td>
                                                ...
                                                <?php if(($roleId<3)|| $currentUser['id']==$user['id']): ?>
                                                <td>
                                                    <a href="./deal/delete.php?collectId=<?php echo$v['id'];?>" class="deletion"><span> 删除</span></a>
                                                </td>
                                                <?php endif;?>
                                                </tr>
                                            <?php endforeach;?>
...
<script type="text/javascript">
    $('#bid1').click(function(){
        ...
        $.post('./deal/showCollectl.php',{userId:userId,number:num,type:type},function(data){
            //2ajax请求
            if(data.code == 200){
                //正常返回数据
