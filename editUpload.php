        ...
        <div class="col-md-12 column">
            ...
            <div class="container projects">
                <div class="projects-header page-header">
                    <span class="h3">上传资源编辑</span>
                </div>
                <!--文件编辑框-->
                <div id="register_body" class="row">
                    <div   class="col-md-6 col-md-offset-3">
                        <form class="form-horizontal" action="./deal/editUpload.php" method="post" enctype="multipart/form-data" id="register-form" >
                            <div class="form-group">
                                <label for="inputDes" class="col-sm-2 control-label">名称</label>
                                <div class="col-sm-10">
                                    <input id='uploadId' type="hidden" name="uploadId"  value="<?php echo $id;?>">
                                    <input id='name'  name="name" type="text" class="form-control" placeholder="请输入文件名称" value="<?php echo $upload['name']?>">
                                </div>
                            </div>
                            <div class="form-group">
                                ...
                                <div class="col-sm-10">
                                    <select class="form-control" name="category">
                                        <?php foreach($category as $v):?>
                                            <?php if( $v['id']==$upload['category_id']):?>
                            ...
                            <div class="form-group">
                                <label for="inputDes" class="col-sm-2 control-label">摘要</label>
                                <div class="col-sm-10">
                                    <textarea id='des' name="des" class="form-control" rows="2"  placeholder="不超过30字"><?php echo $upload['des'];?></textarea>
                                </div>
                            </div>
                            <div class="form-group">
                                <label for="inputDes" class="col-sm-2 control-label">详情介绍</label>
                                <div class="col-sm-10">
                                    <textarea id='detail' name="detail" class="form-control" rows="5"  placeholder="不超过300字"><?php echo $upload['detail'];?></textarea>
                                </div>
                            </div>
                            <div class="form-group">
                                <label for="inputPassword3" class="col-sm-2 control-label">价值</label>
                                <div class="col-sm-10">
                                    <input id='price'  name="price" type="text" class="form-control" placeholder="请输入文件价值" value="<?php echo $upload['price'];?>">
                                </div>
                            </div>
                            <div class="form-group">
...
<script type="text/javascript">
    $(function () {
        $('#register-form').submit(function () {
            ...
            if ( des == '' ||  des.length <= 0 ||  des.length >= 60) {
                layer.tips('摘要规格不符合', '#des', {time: 2000, tips: 2});
                $('#des').focus();
                return false;
            }
            if (detail == '' ||  detail.length <= 0 ||  detail.length >= 900) {
                layer.tips('详情规格不符合', '#detail', {time: 2000, tips: 2});
                $('#email').focus();
                return false;
            }
            if ( price == '' ||  price.length <= 0 || isNaN(price)) {
                layer.tips('价值不能为空且为正整数', '#price', {time: 2000, tips: 2});
                $('#email').focus();
                return false;
            }
            return true;
