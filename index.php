...
{   $limitId=$_GET['id'];
    $limit=" AND p.`category_id`={$limitId}";
}
$search='';
$search1='';
if(!empty($_POST['search']))
{
    ...
    $search=" HAVING  p.`name`LIKE '%{$search1}%' OR  p.`des` LIKE '%{$search1}%'";
}
$sql='SELECT p.`id`, p.`name`,u.`name`as user_name, p.`user_id`, p.`category_id`,
 p.`path`, p.`des`,p. `detail`,p. `price`,p. `view`, p.`create_time` 
 FROM `upload` AS p,users AS u WHERE p.user_id=u.id '.$limit.$search.' limit 0,10';
$t=$s->select($sql);
$uploads=$t['result'];

$sql='SELECT p.`id`, p.`name`,u.`name`as user_name, p.`user_id`, p.`category_id`,
 p.`path`, p.`des`,p. `detail`,p. `price`,p. `view`, p.`create_time` 
 FROM `upload` AS p,users AS u WHERE p.user_id=u.id '.$limit.$search.' 
        ...
        <div class="col-md-12 column">
            <nav class="navbar navbar-default" role="navigation">
                ...
                <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
                    ...
                    <form class="navbar-form navbar-left" action="index.php<?php echo '?id='.$limitId;?>" role="search" method="post">
...
<script type="text/javascript">
    $('#bid1').click(function(){
        var num =  $('.tr1').length - 1,
            search1='<?php echo $search1?>',
            limit='<?php echo $limit?>';
        $.post('./deal/index1.php',{number:num,search1:search1,limit:limit},function(data){
            //2ajax请求
            if(data.code == 200){
                //正常返回数据
